...
(function () {
  ...
  /* -------------------------------------------- carregar lib do Supabase */
  /* Tenta via <script> (jsdelivr), com fallback para unpkg e esm.sh.
     Evita ficar travado para sempre se um CDN estiver bloqueado. */
  ...
  async function loadSupabase() {
    ...
    // última tentativa: módulo ES
    try {
      const mod = await import("https://esm.sh/@supabase/supabase-js@2");
      if (mod && mod.createClient) return mod.createClient;
    } catch (e) { /* ignora */ }
    throw new Error("Não foi possível carregar a biblioteca do Supabase. Verifique sua conexão e recarregue a página.");
  }

  /* ----------------------------------------------------------------- init */
  async function init() {
    if (!window.SUPABASE_URL || !window.SUPABASE_ANON_KEY) {
      alert("config.js não carregou. Verifique se o arquivo está na mesma pasta do index.html.");
      return;
    }

    // 1) a tela de login aparece IMEDIATAMENTE, antes de carregar a lib
    const box = overlay();
    ...
    // 2) só então carrega a biblioteca
    let sb;
    try {
  ...
  function start() {
    init().catch((e) => {
      console.error("[auth.js] falha na inicialização:", e);
      const msg = document.querySelector("#auth-msg");
      if (msg) { msg.textContent = e.message || String(e); msg.className = "err"; }
      else alert("Erro ao iniciar o login: " + (e.message || e));
    });
  }
